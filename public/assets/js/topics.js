// public/assets/js/topics.js
export const topics = {
  // ===== Categorías raíz =====
  animales: [
    "perro","gato","elefante","león","tigre","oso","conejo","pájaro","pez","caballo","serpiente","pato",
    "cocodrilo","jirafa","cebra","hipopótamo","rinoceronte","mono","lobo","zorro","ciervo","ardilla","ratón",
    "murciélago","delfín","ballena","foca","koala","canguro","panda","águila","búho","pingüino","flamenco",
    "loro","cisne","gallina","gallo","pavo","tortuga","iguana","lagarto","camaleón","rana","sapo","salamandra",
    "tiburón","pulpo","medusa","estrella de mar","cangrejo","abeja","mariposa","hormiga","araña","libélula","escarabajo"
  ],
  ...
  "Fútbol": {
    ...
    // Se rellena luego (abajo) con la suma de listas
    "👥 Jugadores (todos)": []
  }
};

// ===== Post-procesado: normaliza y elimina alias que duplican en el UI =====
(() => {
  const F = topics["Fútbol"];
  if (!F) return;

  // Asegurar "⭐ Leyendas" y remover la variante entre paréntesis del UI
  if (F["⭐ Leyendas (retirados)"] && !F["⭐ Leyendas"]) {
    F["⭐ Leyendas"] = F["⭐ Leyendas (retirados)"];
  }
  delete F["⭐ Leyendas (retirados)"];

  // Construir 👥 Jugadores (todos) = actuales + liga arg + leyendas (únicos)
  const ALL = [
    ...(F["🌍 Jugadores actuales (mundo)"] || []),
    ...(F["🇦🇷 Liga Argentina (actuales)"] || []),
    ...(F["⭐ Leyendas"] || [])
  ];
  F["👥 Jugadores (todos)"] = Array.from(new Set(ALL));

  // Remover alias que sólo generaban duplicados en el select
  delete F["Jugadores (todos)"];
  delete F["Todos los jugadores"];
  delete F["🏟️ Equipos (Argentina)"];
  delete F["🏟️ Equipos (Internacionales)"];
})();
